...
public class player2 : MonoBehaviour
{
	...
	public void dropItem()
	{
		...
		myPainting.GetComponent<p2pickUp> ().transform.parent = GameObject.Find ("p2pick").transform;
		GameObject.Find("Player2").GetComponent<player2>().isPicked = false;
		GameObject.Find ("Player2").GetComponent<player2> ().anim.SetBool ("carry", false);
	...
	void OnTriggerEnter(Collider col){
		...
		if (col.gameObject.name == "Player1" && isPicked) {
			...
			GameObject.Find ("Player1").GetComponent<player1> ().anim.SetTrigger ("bump");
		}
	}
